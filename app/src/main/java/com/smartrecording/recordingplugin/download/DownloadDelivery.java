...
final class DownloadDelivery {
   ...
   */
  void postProgress(final DownloadRequest request, final long bytesWritten, final long totalBytes, final double aaa) {
    downloadPoster.execute(new Runnable() {
      @Override public void run() {
        request.downloadCallback().onProgress(request.downloadId(), bytesWritten, totalBytes,  aaa);
      }
    });
  }
